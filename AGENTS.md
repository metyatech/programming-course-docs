<!--
  Generated by agent-rules/tools/compose-agents.cjs.
...
## Private リポジトリ（機密・取り扱い）

- Private リポジトリの情報（ソースコード、仕様、URL、組織名、内部用語、課題内容、ログ等）は機密として扱う。
- 秘密情報（トークン/鍵/パスワード/接続文字列/個人情報など）を出力しない。必要ならマスクして示す。
- 変更差分やログに機密が混入しないように注意し、疑わしい場合は「伏せる/削除する/確認する」を優先する。
- 外部公開が前提の手順（公開Issue/公開Gist/外部サービスへの貼り付け等）を提案しない。共有が必要な場合は、組織の承認済み手段に限定する。
